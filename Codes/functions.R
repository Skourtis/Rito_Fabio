...
DEP_DDA <- function(input_matrix,dataset_name){
    ...
    for(i in (dep@elementMetadata %>% names() %>% str_subset("diff") )){
        ...
        #     ggsave(here::here(output_folder, glue::glue(i," ",dataset_name," ALL-GSEA.png")), height = 20, width  = 15)}
    ...
    ggsave(here::here(output_folder, glue::glue(i," ",dataset_name,"Combined_Cyto_Nucle_FC.png")), height = 20, width  = 15)
    ...
    write.xlsx(data_matrices[1:2], here::here("Datasets","Processed",glue::glue(dataset_name, "matrices.xlsx")), overwrite = T)
